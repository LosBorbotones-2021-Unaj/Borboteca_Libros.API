...
namespace Borboteca_Libros.API.Controllers
{
    ...
    public class LibroController : Controller
    {
        ...
        public IActionResult FiltroLibrosAutor(string busqueda, Guid LibroGuid)
        {
            try
            {
                return new JsonResult(_service.FiltroLibrosAutor(busqueda, LibroGuid)) { StatusCode = 200 };
            }
            catch (Exception e)
            {
                return BadRequest(new { error = "no hay coincidencias de libros con la busqueda realizada" });
